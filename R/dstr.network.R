...
dstr.network <- function(githublink = NULL, pkg = NULL,includebasepkgs = F, recursive = T,which = c("Imports", "Depends")) {
  
  #create edgelist
  edgelist <- nthlvldep(githublink = githublink, pkg = pkg, outtype="edgelistdetailed", includebasepkgs = includebasepkgs, recursive = recursive, which= which)
